...
export default function FormAttestato({ attestato, onSalva, onChiudi }: {
  ...
  const [form, setForm] = useState({
    dipendente_id: '', azienda_id: '', tipologia: '', nome_corso: '',
    protocollo: '', data_inizio: '', data_fine: '', data_attestato: '', data_scadenza: '', note: '',
  })
  ...
  function MiniForm({ titolo, colore, campi, onSalva, onAnnulla }: any) {
    return (
      <div style={{ gridColumn: '1 / -1', background: '#f0f9ff', border: `1px solid #bae6fd`, borderRadius: '10px', padding: '16px' }}>
        <div style={{ fontSize: '13px', fontWeight: '700', color: colore, marginBottom: '12px' }}>{titolo}</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
          {campi}
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={onSalva} style={{ background: colore, color: 'white', border: 'none', borderRadius: '7px', padding: '8px 16px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
            Salva
          </button>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: '16px' }}>
      <div style={{ background: 'white', borderRadius: '16px', width: '100%', maxWidth: '640px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: '17px', fontWeight: '700', color: '#0f172a' }}>{attestato ? 'Modifica Attestato' : 'Nuovo Attestato'}</h2>
